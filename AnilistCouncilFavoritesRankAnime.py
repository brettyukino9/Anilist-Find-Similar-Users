...
users = {}
queries = {}
pages = 5 #1200 users per page, 24 a's on a page, 50 users per a
...
# Make the HTTP Api request to get the users favorite anime
...
#Get user favorites
for x in range(1,3) : 
    for i in response2['data']['User']['favourites']['anime'+str(x)]['nodes']:
        anime.append(i['id'])
...
for x in range(1, pages * 25 - (pages -1 )) :
    ...
    for user in users[num]['data']['a'+str(x)]['following'] :
      ...
      count +=1
      for y in range(1,2) :
          for i in user['favourites']['anime'+str(y)]['nodes']:
              a = i['id']
              if(a in anime) :
                  if(name in map) :
                      map[name] = map[name] + 1
...
with open('Anime Results/'+username2+' results.txt', 'w', encoding='utf8') as f:
    f.write("Number of shared favorite anime between " + username2+" and x user")
    f.write("-----------------------------------------------------------------------\n")
